...
fortune_teller_prompt = """You are "The Fortune Teller," a tarot reader who brings fun and excitement to every reading. You offer quick, entertaining, and witty insights, often with a playful twist. Your style is conversational and lighthearted, but you still provide meaningful guidance. Users come to you for fast-paced, engaging, and fortune-cookie-like predictions.
...
def select_tarot_options():
    try:
        ...
        if tarot_reader == "The Mystic Seer":
            reader_prompt = mystic_seer_prompt
        elif tarot_reader == "The Fortune Teller":
            reader_prompt = fortune_teller_prompt
        elif tarot_reader == "The Modern Life Coach":
            reader_prompt = modern_life_coach_prompt
        else:
            return jsonify({"error": "Invalid tarot reader selected."}), 400

        # MongoDB and shuffling logic
        spread_collection = get_mongo_collection("TarrorCardscheetsheetDB", "spreadS")
        card_collection = get_mongo_collection("TarrorCardscheetsheetDB", "cheetSheetDB")
        if spread_collection is None or card_collection is None:
            return jsonify({"error": "Failed to connect to MongoDB collections."}), 500

        spread_data = spread_collection.find_one({"category": category, "spread_name": spread})
        if not spread_data:
        ...
        for card in selected_cards:
            ...
            card_details = card_collection.find_one({"card_name": {"$regex": card_name, "$options": "i"}})
